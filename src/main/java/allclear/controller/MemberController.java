...
public class MemberController {
    ...
    public ApiResponse emailIsValid(@RequestBody EmailIsValidRequestDto emailIsValidRequestDto) {
        ...
        if (isValid) return ApiResponse.onSuccess("이메일 인증에 성공했습니다");
        else return ApiResponse.onFailure(GlobalErrorCode._INVALID_CODE);
    }
    ...
    public ApiResponse updateGradeAndCurriculum(
            @PathVariable Long userId,
            @RequestBody UpdateMemberRequestDto updateMemberRequestDto,
            @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
