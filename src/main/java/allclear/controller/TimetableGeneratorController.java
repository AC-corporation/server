...
public class TimetableGeneratorController {
    ...
    public ApiResponse step1(
            @PathVariable Long userId,
            @RequestBody Step1RequestDto requestDto,
    ...
    public ApiResponse step2(
            @PathVariable Long userId,
            @RequestBody Step2RequestDto requestDto,
            @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
    ...
    public ApiResponse step3(
            @PathVariable Long userId, @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step3 성공했습니다", timetableGeneratorService.suggestMajorSubject(userId));
    }

    @Operation(summary = "시간표 생성기에 과목 추가", description = "생성할 시간표에 선택한 과목 등록")
    @PostMapping("/step3to7/{userId}")
    public ApiResponse addTimetableGeneratorSubject(
            @PathVariable Long userId,
            @RequestBody Step3to7RequestDto requestDto,
            @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
    ...
    public ApiResponse step4(
            @PathVariable Long userId, @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step4 성공했습니다", timetableGeneratorService.suggestLiberalArtsSubject(userId));
    }

    // ==step5==//
    @Operation(summary = "전공 선택 과목 추천")
    @GetMapping("/step5/{userId}")
    public ApiResponse step5(
            @PathVariable Long userId, @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step5 성공했습니다", timetableGeneratorService.suggestMajorElectiveSubject(userId));
    }

    // ==step6==//
    @Operation(summary = "교양 선택 과목 추천")
    @GetMapping("/step6/{userId}")
    public ApiResponse step6(
            @PathVariable Long userId, @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step6 과목 추천 성공했습니다",
                timetableGeneratorService.suggestLiberalArtsElectiveSubject(userId));
    }
    ...
    public ApiResponse step7(
        ...
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step7 성공했습니다", timetableGeneratorService.getTimetableGeneratorSubjectList(userId));
    }
    ...
    public ApiResponse step7(
            @PathVariable Long userId,
            @RequestBody Step7RequestDto requestDto,
            @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
    ...
    public ApiResponse step8(
        ...
        return ApiResponse.onSuccess(
                "step8 성공했습니다",
                timetableGeneratorService.getTimetableGeneratorTimetableList(userId, page));
    }
    ...
    public ApiResponse step8(
            @PathVariable Long userId,
            @RequestBody Step8RequestDto requestDto,
            @RequestHeader("Authorization") String authorizationHeader) {
        if (!jwtTokenProvider.compareMember(authorizationHeader, userId))
            return ApiResponse.onFailure(GlobalErrorCode._UNAUTHORIZED);
        return ApiResponse.onSuccess(
                "step8 성공했습니다", timetableGeneratorService.saveTimetable(userId, requestDto));
    }

    @Operation(summary = "시간표 생성기 과목 삭제")
    @DeleteMapping("/{timetableGeneratorSubjectId}")
    public ApiResponse deleteTimetableGeneratorSubjectId(
            @PathVariable Long timetableGeneratorSubjectId) {
