...
public class TimetableGeneratorService {
    ...
    /** 시간표 생성기 과목 추가 - 커스텀 과목 Step2 Post */
    public void addCustomTimetableGeneratorSubjects(Long userId, Step2RequestDto requestDto) {
        ...
        for (TimetableSubjectRequestDto timetableSubjectRequestDto :
                requestDto.getTimetableSubjectRequestDtoList()) {
            List<TimetableGeneratorClassInfo> timetableGeneratorClassInfoList = new ArrayList<>();

            for (ClassInfoRequestDto classInfoRequestDto :
                    timetableSubjectRequestDto.getClassInfoRequestDtoList()) {
                timetableGeneratorClassInfoList.add(
            ...
            TimetableGeneratorSubject timetableGeneratorSubject =
                    TimetableGeneratorSubject.createCustomTimetableGeneratorSubject(
    ...
    public Step3to6ResponseDto suggestMajorSubject(Long userId) {
        ...
        if (LocalDate.now().getMonthValue() < 3 && member.getSemester() == 2) memberLevel++;
        // 조회할 과목 전공 저장
        ...
        List<Subject> subjectList =
                new ArrayList<>(
                        subjectRepository.findAll(
                                SubjectSpecification.subjectFilter(
        ...
        // 졸업요건 조회
        List<RequirementComponent> requirementComponentList =
                member.getRequirement().getRequirementComponentList().stream()
                        .filter(
                                requirementComponent ->
                                        (requirementComponent
                                                        ...
                                                        || requirementComponent
                                                                ...
                                                                .equals("전공"))
                                                && !requirementComponent
                                                        .getRequirementArgument()
                                                        .contains("전선"))
                        .toList();
        return new Step3to6ResponseDto(requirementComponentList, subjectList);
    }

    /** 시간표 생성기 과목 추가 - 실제 과목 Step3~7 Post */
    public void addActualTimetableGeneratorSubjects(Long userId, Step3to7RequestDto requestDto) {
        TimetableGenerator timetableGenerator = findById(userId);

        // 중복 선택된 과목 제외
        List<Long> selectedSubjectId =
                timetableGenerator.getTimetableGeneratorSubjectList().stream()
                        .filter(
                                timetableGeneratorSubject ->
                                        timetableGeneratorSubject.getSubject() != null)
                        .map(
                                timetableGeneratorSubject ->
                                        timetableGeneratorSubject.getSubject().getSubjectId())
                        .toList();
        List<Subject> subjectList =
                subjectRepository.findAllById(requestDto.getSubjectIdList()).stream()
        ...
        for (Subject subject : subjectList) {
            isMajor =
                    subject.getMajorClassification()
                                    .contains(
                                            majorConvertor(
                                                    timetableGenerator.getMember().getMajor()))
            ...
            TimetableGeneratorSubject timetableGeneratorSubject =
                    TimetableGeneratorSubject.createActualTimetableGeneratorSubject(
                            subject, isMajor);
            timetableGenerator.addTimetableGeneratorSubject(timetableGeneratorSubject);
        }
    }
    ...
    public Step3to6ResponseDto suggestLiberalArtsElectiveSubject(Long userId) {
        ...
        for (RequirementComponent requirementComponent : requirementComponentList) {
            if (requirementComponent.getRequirementArgument().contains("통합조건")
                ...
                String liberalArt =
                        liberalArtConvertor(requirementComponent.getRequirementArgument());
                subjectList.addAll(
                        (subjectRepository.findAll(
                                SubjectSpecification.subjectFilter(
                                        SubjectSpecification.builder()
    ...
    /** 선택한 과목 리스트 제공 Step7 get */
    public Step7ResponseDto getTimetableGeneratorSubjectList(Long userId) {
        List<TimetableGeneratorSubject> timetableGeneratorSubjectList =
                findById(userId).getTimetableGeneratorSubjectList();
    ...
    /** 시간표 생성 (미완성) Step 7 Post */
    public void generateTimetableList(Long userId, Step7RequestDto requestDto) {
        TimetableGenerator timetableGenerator = findById(userId);
        // 시간표 생성
        List<TimetableGeneratorTimetable> newTGTimetableList =
                generateTimetables(timetableGenerator, requestDto);
    ...
    public Step8ResponseDto getTimetableGeneratorTimetableList(Long userId, int page) {
        ...
        Page<TimetableGeneratorTimetable> timetablePage =
                tgTimetableRepository.findAllByTimetableGenerator(timetableGenerator, pageable);
    ...
    /** 생성한 시간표 저장 Step8 Post */
    public Long saveTimetable(Long userId, Step8RequestDto requestDto) {
        // 생성할 시간표를 시간표 생성기에서 찾아오기
        TimetableGeneratorTimetable generatorTimetable =
                tgTimetableRepository
                        .findById(requestDto.getTimetableGeneratorTimetableId())
                        .orElseThrow(() -> new GlobalException(GlobalErrorCode._NO_CONTENTS));
        Member member =
                memberRepository
                        .findById(userId)
        ...
        // 시간표에 과목 추가
        for (TimetableGeneratorSubject generatorSubject :
                generatorTimetable.getTimetableGeneratorTimetableSubjectList().stream()
            ...
            if (generatorSubject.getSubject() == null) {
                ...
                for (TimetableGeneratorClassInfo generatorClassInfo :
                        generatorSubject.getTimetableGeneratorClassInfoList()) {
                ...
                timetableSubject =
                        TimetableSubject.createCustomTimeTableSubject(
                                generatorSubject.getSubjectName(), timetableClassInfoList);
            } else {
                // 실제 과목 생성
                timetableSubject =
                        TimetableSubject.createActualTimeTableSubject(
                                generatorSubject.getSubject());
            }
            // 생성한 과목 시간표에 추가
    ...
    public void deleteTimetableGeneratorSubject(Long timetableGeneratorSubjectId) {
        tgSubjectRepository
                .findById(timetableGeneratorSubjectId)
                .orElseThrow(() -> new GlobalException(GlobalErrorCode._NO_CONTENTS));
    ...
    private String majorConvertor(String major) {
        // IT 대학
        return major.contains("컴퓨터")
                ? "컴퓨터"
                : major.contains("소프트")
                        ? "소프트"
                        : major.contains("AI융합")
                                ? "AI융합"
                                : major.contains("글로벌미디어")
                                        ? "글로벌미디어"
                                        : major.contains("글로벌미디어")
                                                ? "글로벌미디어"
                                                : major.contains("미디어경영")
                                                        ? "미디어경영"
                                                        : major.contains("정보보호")
                                                                ? "정보보호"
                                                                : major.contains("전자정보공학부 it융합")
                                                                        ? "IT융합"
                                                                        : major.contains(
                                                                                        "전자정보공학부 전자공학")
                                                                                ? "전자공학"
    ...
    private void removeUnnecessarySubject(List<Subject> subjectList, List<Long> prevSubjectIdList) {
        ...
        for (Subject subject : subjectList)
            if (uniqueSubjectIds.add(subject.getSubjectId())) uniqueSubjects.add(subject);
        subjectList.clear();
    ...
    private void sortSubject(List<Subject> subjectList, String major, int level) {
        subjectList.sort(
                Comparator.comparing((Subject o) -> o.getSubjectTarget().contains(major) ? 0 : 1)
                        .thenComparing(
                                o -> o.getSubjectTarget().contains(String.valueOf(level)) ? 0 : 1));
    }

    // 교과영역 문자열 변환
    private String liberalArtConvertor(String liberalArt) {
        return liberalArt.contains("공동체/리더십")
                ? "공동체/리더십"
                : liberalArt.contains("의사소통/글로벌")
                        ? "의사소통/글로벌"
                        : liberalArt.contains("창의융합") ? "창의/융합" : liberalArt;
    }
}
